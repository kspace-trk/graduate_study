...
const one_measure_repeating_notes_diff = []
const two_measure_repeating_notes_diff = []
const params = {
...
const parse_json = async () => {
  await fs.readdir('./input_data', (files) => {
    files.forEach((file) => {
      if (file !== '.DS_Store') {
        input_notes.push(
...
const name_conversion = (element, scale) => {
  ...
  const base_name_num = calc_base_name_num(element.notes) // 基準となる音高の数値を算出
  element.notes.forEach((elem_input_notes) => {
    const sliced_name = elem_input_notes.name.slice(0, -1) // 音高のアルファベット
...
const output = () => {
  const one_measure_repeating_notes_json = JSON.stringify(
    { notes: one_measure_repeating_notes },
    null,
    ''
  )
  const two_measure_repeating_notes_json = JSON.stringify(
    { notes: two_measure_repeating_notes },
    null,
    ''
  )
  const four_measure_repeating_notes_json = JSON.stringify(
...
const check_time_mutation_start_point = (elem_song, compartion) => {
  ...
  if (compartion === 2) {
    ...
    elem_song[0].time.forEach((elem_one_measure_note, index) => {
      if (
        elem_one_measure_note !== elem_song[2].time[index] - 4 &&
        tmp === null
      ) {
        tmp = index / elem_song[0].time.length
        time_mutation_start_point = Math.round(tmp * 100) / 100
  ...
  if (compartion === 3) {
    ...
    elem_song[0].time.forEach((elem_one_measure_note, index) => {
      if (
        elem_one_measure_note !== elem_song[3].time[index] - 6 &&
        tmp === null
      ) {
        tmp = index / elem_song[0].time.length
        time_mutation_start_point = Math.round(tmp * 100) / 100
...
const check_name_mutation_start_point = (elem_song, compartion) => {
  let name_mutation_start_point = null
  if (compartion === 1) {
    let tmp = null
    elem_song[0].name.forEach((elem_one_measure_note, index) => {
      if (elem_one_measure_note !== elem_song[1].name[index] && tmp === null) {
        tmp = index / elem_song[0].name.length
        name_mutation_start_point = Math.round(tmp * 100) / 100
      }
    })
  }
  if (compartion === 2) {
    let tmp = null
    elem_song[0].name.forEach((elem_one_measure_note, index) => {
      if (elem_one_measure_note !== elem_song[2].name[index] && tmp === null) {
        tmp = index / elem_song[0].name.length
        name_mutation_start_point = Math.round(tmp * 100) / 100
      }
    })
  ...
  if (compartion === 3) {
    let tmp = null
    elem_song[0].name.forEach((elem_one_measure_note, index) => {
      if (elem_one_measure_note !== elem_song[3].name[index] && tmp === null) {
        tmp = index / elem_song[0].name.length
        name_mutation_start_point = Math.round(tmp * 100) / 100
      }
    })
  }
  return name_mutation_start_point
}
const calc_mutation_start_point = (elem_song, note_diff, repeating_time) => {
  ...
  if (elem_song[0].time.length === elem_song[1].time.length) {
    ...
    note_diff.name_mutation_start_point.push(
      check_name_mutation_start_point(elem_song, compartion)
    )
  } else {
...
const calc_note_diff = () => {
  one_measure_repeating_notes.forEach((elem_song) => {
    ...
    const note_diff = {
      ...
      name_mutation_start_point: [null]
    }
...
const output_diff = () => {
  ...
  const one_measure_repeating_notes_diff_json = JSON.stringify(
    { notes: one_measure_repeating_notes_diff },
    null,
    ''
  )
  const two_measure_repeating_notes_diff_json = JSON.stringify(
